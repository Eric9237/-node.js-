...
const validate = require('../middleware/validate_idea');
const { add_idea_schema } = require('../schema/add_idea')
//新增
router.post('/add', validate(add_idea_schema), (req, res) => {

    const sql = 'insert into 05_idea (content) values (?)';
    db.query(sql, [req.body.content], (err, results) => {
        if (err) return res.cc(err);

        const selSql = 'select * from 05_idea ORDER BY id DESC'
...
router.get('/delete', (req, res) => {
    let id = req.query.id
    const delSql = "delete from 05_idea where id = ?"
    const selSql = 'select * from 05_idea ORDER BY id DESC'
...
router.get('/updataDATA', (req, res) => {
    ...
    const sql = 'select * from 05_idea where id = ?'
...
// 修改 updata
router.post('/updata', validate(add_idea_schema), (req, res) => {
