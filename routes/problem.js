...
const validate = require('../middleware/validate_problem.js');
const { add_problem_schema } = require('../schema/add_problem')
//新增
router.post('/add', validate(add_problem_schema), (req, res) => {

    const sql = 'insert into 04_problem (content) values (?)';
    db.query(sql, [req.body.content], (err, results) => {
        if (err) return res.cc(err);

        const selSql = 'select * from 04_problem ORDER BY id DESC'
...
router.get('/delete', (req, res) => {
    let id = req.query.id
    const delSql = "delete from 04_problem where id = ?"
    const selSql = 'select * from 04_problem ORDER BY id DESC'
...
router.get('/updataDATA', (req, res) => {
    ...
    const sql = 'select * from 04_problem where id = ?'
...
// 修改 updata
router.post('/updata', validate(add_problem_schema), (req, res) => {
